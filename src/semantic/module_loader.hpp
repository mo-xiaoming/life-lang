...
struct Module_Descriptor {
  ...
  std::filesystem::path directory;           // Filesystem directory path containing module files
  std::vector<std::filesystem::path> files;  // Filesystem paths to all .life files in the module

  // Get simple module name (last component of path)
  [[nodiscard]] std::string name() const { return path.empty() ? "" : path.back(); }

  // Get dot-separated module path string (e.g., "Std.Collections")
  [[nodiscard]] std::string module_path_string() const;
