...
namespace life_lang::ast::detail {

void Sexp_Printer::write(std::string_view text_) {
  maybe_indent();
  m_oss << text_;
}

void Sexp_Printer::maybe_indent() {
  if (m_needs_indent && m_indent_size > 0) {
    m_oss << '\n' << std::string(static_cast<std::size_t>(m_depth * m_indent_size), ' ');
    m_needs_indent = false;
  }
}

void Sexp_Printer::begin_list(std::string_view tag_) {
  maybe_indent();
  m_oss << '(' << tag_;
  ++m_depth;
  m_needs_indent = true;
...
void Sexp_Printer::write_quoted(std::string_view str_) {
  maybe_indent();
  m_oss << escape_string(str_);
}

void Sexp_Printer::write_bool(bool value_) {
  maybe_indent();
...
namespace {
// Forward declarations for all print_sexp overloads (used for mutual recursion)
...
void print_sexp(Sexp_Printer& p_, Block const& block_);
void print_sexp(Sexp_Printer& p_, If_Expr const& if_expr_);
void print_sexp(Sexp_Printer& p_, While_Expr const& while_expr_);
void print_sexp(Sexp_Printer& p_, For_Expr const& for_expr_);
void print_sexp(Sexp_Printer& p_, Match_Expr const& match_expr_);
void print_sexp(Sexp_Printer& p_, Module const& mod_);

// Type system S-expression printers
void print_sexp(Sexp_Printer& p_, Type_Name_Segment const& seg_) {
...
void print_sexp(Sexp_Printer& p_, Path_Type const& path_) {
  p_.begin_list("path");
...
void print_sexp(Sexp_Printer& p_, Function_Type const& func_) {
...
void print_sexp(Sexp_Printer& p_, Array_Type const& arr_) {
...
void print_sexp(Sexp_Printer& p_, Tuple_Type const& tuple_) {
...
void print_sexp(Sexp_Printer& p_, Trait_Bound const& bound_) {
...
void print_sexp(Sexp_Printer& p_, Type_Param const& param_) {
...
void print_sexp(Sexp_Printer& p_, Where_Predicate const& pred_) {
...
void print_sexp(Sexp_Printer& p_, Where_Clause const& where_) {
  p_.begin_list("where");
...
void print_sexp(Sexp_Printer& p_, Var_Name_Segment const& seg_) {
...
void print_sexp(Sexp_Printer& p_, Var_Name const& var_) {
  p_.begin_list("var");
...
void print_sexp(Sexp_Printer& p_, String const& str_) {
...
void print_sexp(Sexp_Printer& p_, String_Interp_Part const& part_) {
  std::visit(
...
void print_sexp(Sexp_Printer& p_, String_Interpolation const& interp_) {
...
void print_sexp(Sexp_Printer& p_, Integer const& i_) {
...
void print_sexp(Sexp_Printer& p_, Float const& f_) {
  p_.begin_list("float");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Char const& ch_) {
  p_.begin_list("char");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Bool_Literal const& bool_) {
  p_.begin_list("bool");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Unit_Literal const& /*unused*/) {
  p_.write("unit");
}

void print_sexp(Sexp_Printer& p_, Field_Initializer const& field_) {
...
void print_sexp(Sexp_Printer& p_, Struct_Literal const& lit_) {
...
void print_sexp(Sexp_Printer& p_, Array_Literal const& lit_) {
...
void print_sexp(Sexp_Printer& p_, Tuple_Literal const& lit_) {
...
void print_sexp(Sexp_Printer& p_, Binary_Op op_) {
  switch (op_) {
    case Binary_Op::Add:
      p_.write("+");
      break;
...
void print_sexp(Sexp_Printer& p_, Unary_Op op_) {
  switch (op_) {
    case Unary_Op::Neg:
      p_.write("-");
      break;
...
void print_sexp(Sexp_Printer& p_, Binary_Expr const& expr_) {
...
void print_sexp(Sexp_Printer& p_, Unary_Expr const& expr_) {
  p_.begin_list("unary");
  p_.space();
  print_sexp(p_, expr_.op);
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Range_Expr const& expr_) {
...
void print_sexp(Sexp_Printer& p_, Cast_Expr const& expr_) {
  p_.begin_list("cast");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Func_Call_Expr const& call_) {
  p_.begin_list("call");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Field_Access_Expr const& access_) {
...
void print_sexp(Sexp_Printer& p_, Index_Expr const& index_) {
  p_.begin_list("index");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Assignment_Statement const& assign_) {
...
void print_sexp(Sexp_Printer& p_, Wildcard_Pattern const& /*unused*/) {
  p_.write("_");
}

void print_sexp(Sexp_Printer& p_, Literal_Pattern const& lit_) {
...
void print_sexp(Sexp_Printer& p_, Simple_Pattern const& simple_) {
...
void print_sexp(Sexp_Printer& p_, Field_Pattern const& field_) {
...
void print_sexp(Sexp_Printer& p_, Struct_Pattern const& struct_pat_) {
...
void print_sexp(Sexp_Printer& p_, Tuple_Pattern const& tuple_) {
...
void print_sexp(Sexp_Printer& p_, Enum_Pattern const& enum_pat_) {
...
void print_sexp(Sexp_Printer& p_, Or_Pattern const& or_pat_) {
...
void print_sexp(Sexp_Printer& p_, Let_Statement const& let_) {
  p_.begin_list("let");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Func_Call_Statement const& call_) {
...
void print_sexp(Sexp_Printer& p_, Expr_Statement const& expr_) {
...
void print_sexp(Sexp_Printer& p_, Return_Statement const& ret_) {
...
void print_sexp(Sexp_Printer& p_, Break_Statement const& brk_) {
  p_.begin_list("break");
  if (brk_.value) {
    p_.space();
...
void print_sexp(Sexp_Printer& p_, Continue_Statement const& /*unused*/) {
  p_.write("continue");
}

void print_sexp(Sexp_Printer& p_, If_Statement const& if_stmt_) {
...
void print_sexp(Sexp_Printer& p_, While_Statement const& while_stmt_) {
...
void print_sexp(Sexp_Printer& p_, For_Statement const& for_stmt_) {
...
void print_sexp(Sexp_Printer& p_, Else_If_Clause const& clause_) {
...
void print_sexp(Sexp_Printer& p_, Match_Arm const& arm_) {
  p_.begin_list("arm");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Func_Param const& param_) {
  p_.begin_list("param");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Func_Decl const& decl_) {
...
void print_sexp(Sexp_Printer& p_, Func_Def const& def_) {
...
void print_sexp(Sexp_Printer& p_, Struct_Field const& field_) {
  p_.begin_list("field");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Struct_Def const& def_) {
...
void print_sexp(Sexp_Printer& p_, Unit_Variant const& variant_) {
...
void print_sexp(Sexp_Printer& p_, Tuple_Variant const& variant_) {
...
void print_sexp(Sexp_Printer& p_, Struct_Variant const& variant_) {
...
void print_sexp(Sexp_Printer& p_, Enum_Variant const& variant_) {
  std::visit([&](auto const& v_) { print_sexp(p_, v_); }, variant_);
}

void print_sexp(Sexp_Printer& p_, Enum_Def const& def_) {
...
void print_sexp(Sexp_Printer& p_, Impl_Block const& impl_) {
  p_.begin_list("impl");
  p_.space();
...
void print_sexp(Sexp_Printer& p_, Assoc_Type_Decl const& assoc_) {
...
void print_sexp(Sexp_Printer& p_, Assoc_Type_Impl const& assoc_) {
...
void print_sexp(Sexp_Printer& p_, Trait_Def const& def_) {
...
void print_sexp(Sexp_Printer& p_, Trait_Impl const& impl_) {
...
void print_sexp(Sexp_Printer& p_, Type_Alias const& alias_) {
  ...
  std::visit(
      [&](auto const& s_) {
        ...
        if constexpr (std::is_same_v<T, std::shared_ptr<Func_Def>> || std::is_same_v<T, std::shared_ptr<Struct_Def>> ||
                      ...
                      std::is_same_v<T, std::shared_ptr<Assignment_Statement>> ||
...
void print_sexp(Sexp_Printer& p_, Import_Item const& item_) {
...
void print_sexp(Sexp_Printer& p_, Import_Statement const& import_) {
...
void print_sexp(Sexp_Printer& p_, Item const& item_) {
  p_.begin_list("item");
  p_.space();
...
template <typename T>
std::string to_sexp_string(T const& node_, int indent_) {
...
template std::string to_sexp_string(Module const&, int);
template std::string to_sexp_string(Item const&, int);
template std::string to_sexp_string(Func_Def const&, int);
template std::string to_sexp_string(Struct_Def const&, int);
template std::string to_sexp_string(Enum_Def const&, int);
template std::string to_sexp_string(Impl_Block const&, int);
template std::string to_sexp_string(Trait_Def const&, int);
template std::string to_sexp_string(Trait_Impl const&, int);
template std::string to_sexp_string(Type_Alias const&, int);
template std::string to_sexp_string(Expr const&, int);
template std::string to_sexp_string(Statement const&, int);
template std::string to_sexp_string(Pattern const&, int);
template std::string to_sexp_string(Type_Name const&, int);
template std::string to_sexp_string(Import_Statement const&, int);
// Additional types needed by tests
template std::string to_sexp_string(Let_Statement const&, int);
template std::string to_sexp_string(Array_Type const&, int);
template std::string to_sexp_string(Block const&, int);
template std::string to_sexp_string(Bool_Literal const&, int);
template std::string to_sexp_string(Char const&, int);
template std::string to_sexp_string(Integer const&, int);
template std::string to_sexp_string(Float const&, int);
template std::string to_sexp_string(Function_Type const&, int);
template std::string to_sexp_string(String const&, int);
