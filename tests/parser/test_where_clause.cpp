...
TEST_CASE("Where clauses in function declarations") {
  SUBCASE("simple where clause") {
    ...
    if (auto const& where_opt = result->declaration.where_clause; where_opt.has_value()) {
      auto const& where = *where_opt;
      REQUIRE(where.predicates.size() == 1);
      REQUIRE(where.predicates[0].bounds.size() == 1);
    }
  }

  SUBCASE("multiple bounds on one type") {
  ...
  SUBCASE("inline bounds and where clause") {
  ...
  SUBCASE("no where clause - regression test") {
...
TEST_CASE("Where clauses in struct definitions") {
  SUBCASE("simple where clause") {
    ...
    if (auto const& where_opt = result->where_clause; where_opt.has_value()) {
      auto const& where = *where_opt;
      REQUIRE(where.predicates.size() == 1);
    }
  }
  ...
  SUBCASE("empty struct with where") {
